...
Dato uno spazio topologico $X$ e un campo $k$, i gruppi di omologia $H_i(X;k)$ per $i\in\N$ sono $k$-spazi vettoriali che codificano proprietà di interesse dello spazio $X$. La dimensione di $H_0$, ad esempio, è il numero di componenti connesse di $X$, $H_1$ "conta" il numero di \emph{buchi $1$-dimensionali}, e così via.

Esistono diversi modi di calcolare questi gruppi. Il più generale è quello di utilizzare l'omologia singolare, tuttavia questa risulta difficilmente calcolabile perché richiede di lavorare con quozienti di spazi vettoriali di dimensione più che numerabile. Il metodo che useremo qui
